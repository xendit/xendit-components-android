...
class SessionResponseTest {
  ...
  fun deserialize_whenSubscriptionSessionPresent_mapsSubscriptionDetails() {
    ...
    assertEquals(BffSubscriptionInterval.MONTH, response.session?.subscription?.schedule?.interval)
    assertEquals(1, response.session?.subscription?.schedule?.intervalCount)
    assertEquals(BffSubscriptionInterval.DAY, response.session?.subscription?.schedule?.retryInterval)
